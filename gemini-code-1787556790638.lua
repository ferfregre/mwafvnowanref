...
-- Логика Анти-аима: если включен Zoom (3-е лицо) и Анти-аим в пол, крутим UpperTorso / Head вниз
RunService.RenderStepped:Connect(function()
    if cameraFOV then
    ...
    if zoomEnabled and Settings.AntiAimDown then
        ...
        if char then
            ...
            local head = char:FindFirstChild("Head")
            if upperTorso then
                -- Наклоняем корпус персонажа вперед/вниз
                upperTorso.CFrame = upperTorso.CFrame * CFrame.Angles(math.rad(90), 0, 0)
            end
            if head then
...
Rayfield:Notify({
    Title = "@set9p",
    Content = "Трейнер удален. Анти-аим добавлен!",
    Duration = 3,
    Image = 4483362458,
})