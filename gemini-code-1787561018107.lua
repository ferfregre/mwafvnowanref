-- ============================================
-- @set9p | SILENT AIM (360°) + AUTO TRIGGER (SPAM) + VISIBLE CHECK + ESP + ANTI-AIM (DOWN)
-- ============================================
...
local Settings = {
    SilentAim = false,
    AimPart = "Head", -- Управляется отдельными тумблерами
    AimFOV = 200,
    Aim360 = false,
    DrawFOV = false,
    
    -- Auto Trigger & Visible Check
    AutoTrigger = false,
    VisibleCheck = true,
...
CombatTab:CreateToggle({
    Name = "Visible Check (Только видимые)",
    CurrentValue = true,
...
-- ОТДЕЛЬНЫЕ НЕЗАВИСИМЫЕ ТУМБЛЕРЫ ДЛЯ ГОЛОВЫ И ТОРСА
CombatTab:CreateToggle({
    Name = "Голова (Head)",
    CurrentValue = true,
    Flag = "AimHeadToggle",
...
Rayfield:Notify({
    Title = "@set9p",
    Content = "Скрипт полностью восстановлен, добавлены отдельные тумблеры хитбокса!",
    Duration = 3,
    Image = 4483362458,
})