-- ============================================
-- SWILL | FULL SCRIPT (FAST CLICK TAPPER)
-- ============================================
...
local target = nil
local lastFire = 0
local fireDelay = 0.03 -- Уменьшил задержку для более быстрого темпа стрельбы
            ...
            if hrp and Drawing and Drawing.new then
                ...
                if onScreen then
                    ...
                    if espShowHealth then
                        ...
                        hb.Color = Color3.fromHSV(healthPercent * 0.3, 1, 1)
                    else
                        hbBg.Visible = false
                        hb.Visible = false
                    end
...
-- ============================
-- FOV КРУГ И СТРЕЛЬБА (БЫСТРЫЕ КЛИКИ ЛКМ)
-- ============================
...
local function shoot()
    ...
    if now - lastFire >= fireDelay then
        lastFire = now
        if VIM then
            -- Быстрый одиночный клик: Нажал -> мгновенно отпустил
...
RunService.RenderStepped:Connect(function()
    if not autofireEnabled and not triggerbotEnabled then return end
    ...
    if autofireEnabled then
        shoot()
    elseif triggerbotEnabled then
        local center = Camera.ViewportSize / 2
        local screenPos, onScreen = Camera:WorldToViewportPoint(targetPart.Position)
        if onScreen then
            local dist = (Vector2.new(screenPos.X, screenPos.Y) - center).Magnitude
            if dist <= aimFOV and isVisible(targetPart) then
                shoot()
            end
        end
    end
...
Rayfield:Notify({
    Title = "Готово",
    Content = "Стрельба переведена на быстрые одиночные клики!",
    Duration = 3,
    Image = 4483362458,
})