-- ============================================
-- @set9p | SILENT AIM + ESP + NO SPREAD + WALLBANG
-- ============================================
...
local Settings = {
    SilentAim = false,
    NoSpread = false,
    Wallbang = false, -- Прострел сквозь стены
...
-- MISC (Bhop, Spin, Zoom, Camera FOV)
...
local function isVisible(targetPart)
    ...
    if not origin or typeof(origin) ~= "CFrame" then return false end
...
local function isEnemy(player)
    ...
    if Status then
        ...
        if Team and LocalStatus then
            local LocalTeam = LocalStatus:FindFirstChild("Team")
            if Team.Value == "Spectator" then return false end
            if LocalTeam and Team.Value == LocalTeam.Value then return false end
        end
...
-- ============================
-- SILENT AIM, NO SPREAD & WALLBANG (HOOK)
-- ============================
    ...
    for _, v in pairs(plrs) do
        ...
        if onScreen then
            ...
            if distance <= Settings.AimFOV and distance < closestDistance then
                if checkVisibility and not isVisible(targetHitPart) then continue end
...
RunService.RenderStepped:Connect(function()
    if Settings.SilentAim or Settings.NoSpread or Settings.Wallbang then
        -- Если включен Wallbang, игнорируем преграды при поиске цели
        local ignoreVis = Settings.Wallbang
        target = GetClosestPlayer(not ignoreVis)
    else
        target = nil
    end
end)
    ...
    pcall(function()
        old = hookfunction(Ray.new, newcclosure(function(origin, direction)
            ...
            if trace:find("Client") and not trace:find("10420") and not trace:find("10595") then
                -- 1. Silent Aim имеет приоритет (если видит в FOV)
                if Settings.SilentAim and target and target:IsA("BasePart") then
                    shotFiredTime = tick()
                    direction = target.Position - origin
                -- 2. Wallbang направляет луч в цель даже сквозь стены
                elseif Settings.Wallbang and target and target:IsA("BasePart") then
                    shotFiredTime = tick()
                    direction = target.Position - origin
                -- 3. NoSpread (антиразброс)
                elseif Settings.NoSpread and target and target:IsA("BasePart") then
                    shotFiredTime = tick()
                    direction = target.Position - origin
                end
            end
...
CombatTab:CreateToggle({
    Name = CanHook and "Антиразброс (No Spread)" or "No Spread [NO WORK]",
    CurrentValue = false,
    Flag = "NoSpreadToggle",
    Callback = function(v) 
        if not CanHook then Settings.NoSpread = false return end
        Settings.NoSpread = v 
    end
})

CombatTab:CreateToggle({
    Name = CanHook and "Wallbang (Прострел стен)" or "Wallbang [NO WORK]",
    CurrentValue = false,
    Flag = "WallbangToggle",
    Callback = function(v) 
        if not CanHook then Settings.Wallbang = false return end
        Settings.Wallbang = v 
    end
})

CombatTab:CreateDropdown({
    Name = "Хитбокс (Aim Part)",
    Options = {"Head", "Torso"},
...
Rayfield:Notify({
    Title = "@set9p",
    Content = "Скрипт с Wallbang успешно загружен!",
    Duration = 3,
    Image = 4483362458,
})