...
-- ============================
-- WALLCAM (ТОЛЬКО ПРОТИВОПОЛОЖНАЯ КОМАНДА)
-- ============================
local function getEnemyPlayers()
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p == LocalPlayer then continue end
        if not p.Team then continue end
        if p.Team == myTeam then continue end
...
frame.Size = UDim2.fromOffset(430, 310)
frame.Position = UDim2.new(0.5, -215, 0.5, -155)
...
local function makeButton(text, col, row, get, set)
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        if text == "WallCam" then toggleWallCam() end
    end)
    return btn
end
...
makeButton("WallCam", 1, 4, function() return Settings.WallCam end, function(v) 
    Settings.WallCam = v
    toggleWallCam()
end)
...
-- ============================
-- Camera FOV Slider
local camFOVLabel = Instance.new("TextLabel")
camFOVLabel.Size = UDim2.new(0, 100, 0, 25)
camFOVLabel.Position = UDim2.new(0, 210, 0, 175)
camFOVLabel.BackgroundTransparency = 1
camFOVLabel.Text = "Cam FOV: " .. cameraFOV
camFOVLabel.TextColor3 = Color3.fromRGB(200, 200, 200)
camFOVLabel.TextSize = 12
camFOVLabel.Font = Enum.Font.GothamMedium
camFOVLabel.Parent = frame

local function makeSliderButton(text, x, callback)
    local btn = Instance.new("TextButton")
    btn.Size = UDim2.fromOffset(30, 25)
    btn.Position = UDim2.new(0, x, 0, 173)
...
makeSliderButton("-", 320, function()
    cameraFOV = math.max(1, cameraFOV - 5)
    camFOVLabel.Text = "Cam FOV: " .. cameraFOV
    updateCameraFOV()
end)

makeSliderButton("+", 355, function()
    cameraFOV = math.min(120, cameraFOV + 5)
    camFOVLabel.Text = "Cam FOV: " .. cameraFOV
    updateCameraFOV()
end)
