...
-- ============================
-- ЭФФЕКТ ЛУЖИ ПРИ ПРЫЖКЕ (FIXED)
-- ============================
local function spawnJumpPuddle(hrp)
    if not jumpPuddleEnabled or not hrp then return end
    
    -- Делаем луч вниз от ног, чтобы найти точное положение земли
    local params = RaycastParams.new()
    params.FilterType = Enum.RaycastFilterType.Exclude
...
local function monitorCharacter(char, player)
    ...
    hum.StateChanged:Connect(function(_, newState)
        if newState == Enum.HumanoidStateType.Jumping then
            -- Запускаем таймер долета до земли (или срабатывания приземления)
...
Rayfield:Notify({
    Title = "Готово",
    Content = "Скрипт полностью исправлен и обновлен!",
    Duration = 3,
    Image = 4483362458,
})