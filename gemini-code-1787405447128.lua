-- ============================================
-- SWILL | FULL SCRIPT (FIXED BHOP, AIM, ESP, CONFIGS)
-- ============================================
...
-- ============================
-- NIGHTMODE, BHOP (ИСПРАВЛЕНО), SPIN, ZOOM
-- ============================
RunService.RenderStepped:Connect(function()
...
-- Исправленный Bhop с отслеживанием респаунов
task.spawn(function()
    while true do
        task.wait()
...
-- ============================
-- СИСТЕМА КОНФИГОВ (С ЛИСТОМ И УДАЛЕНИЕМ)
-- ============================
local ConfigFolderName = "SwillConfigs"
...
local function saveConfig(name)
    ...
    local data = {
        ...
        espFill = espShowFill,
        espRGB = espRGB,
        night = nightModeEnabled,
        worldColor = worldColorEnabled,
        bhop = bhopEnabled,
        zoom = zoomEnabled,
        camFOV = cameraFOV
    }
    writefile(ConfigFolderName .. "/" .. name .. ".json", HttpService:JSONEncode(data))
end
...
local function loadConfig(name)
    ...
    if isfile(path) then
        ...
        if success and type(result) == "table" then
            aimPartOption = result.aimPart or "Head"
            aimFOV = result.aimFOV or 200
            drawFOV = result.drawFOV or false
            espShowBox = result.espBox or true
            espShowName = result.espName or true
            espShowDist = result.espDist or true
            espShowFill = result.espFill or false
            espRGB = result.espRGB or false
            nightModeEnabled = result.night or false
            worldColorEnabled = result.worldColor or false
            updateWorldColor()
            bhopEnabled = result.bhop or false
            zoomEnabled = result.zoom or false
            cameraFOV = result.camFOV or 70
            return true
        end
    end
    return false
...
-- TAB: КОНФИГИ (МЕНЕДЖЕР)
local ConfigTab = Window:CreateTab("Конфиги", 4483362458)

local typedConfigName = "MyConfig"

ConfigTab:CreateInput({
    Name = "Имя нового конфига",
    PlaceholderText = "Введите имя...",
    RemoveTextAfterFocusLost = false,
    Flag = "ConfigNameInput",
    Callback = function(text)
        if text and text ~= "" then
            typedConfigName = text
        end
    end
})

ConfigTab:CreateButton({
    Name = "Создать / Сохранить конфиг",
    Callback = function()
        saveConfig(typedConfigName)
        Rayfield:Notify({ Title = "Конфиги", Content = "Конфиг '" .. typedConfigName .. "' сохранен!", Duration = 3 })
    end
})

local configDropdown = ConfigTab:CreateDropdown({
    Name = "Список конфигов",
    Options = getSavedConfigsList(),
    CurrentOption = getSavedConfigsList()[1] or "Default",
...
ConfigTab:CreateButton({
    Name = "Загрузить выбранный конфиг",
    Callback = function()
        if loadConfig(selectedConfigToManage) then
            Rayfield:Notify({ Title = "Конфиги", Content = "Конфиг '" .. selectedConfigToManage .. "' загружен!", Duration = 3 })
        else
            Rayfield:Notify({ Title = "Конфиги", Content = "Не удалось загрузить конфиг!", Duration = 3 })
        end
    end
})

ConfigTab:CreateButton({
    Name = "Удалить выбранный конфиг",
    Callback = function()
        if deleteConfig(selectedConfigToManage) then
            Rayfield:Notify({ Title = "Конфиги", Content = "Конфиг '" .. selectedConfigToManage .. "' удален!", Duration = 3 })
        else
            Rayfield:Notify({ Title = "Конфиги", Content = "Не удалось удалить файл!", Duration = 3 })
        end
    end
})
...
Rayfield:Notify({
    Title = "Готово",
    Content = "Бхоп, аим в голову, есп и менеджер конфигов исправлены!",
    Duration = 3,
    Image = 4483362458,
})